...
FBStatus.done(function(stat) {
	...
	$('.modal').modal('hide');
	$('.roomodal').modal({backdrop: true}).show();
	...
	$('.newbtn').click(function(){
	    FB.ui({method: 'apprequests',
		   message: 'wants to watch a video with you on Tuki'
		      ...
		      console.log('res', res);
		      window.roomData = {users: $.merge([FB.getUserID()], res.to), id: GUID()};
...
function chatController($scope, $defer) {
	...
	$scope.addMessage = function() {
	    $scope.firebaseRef.push({fbid: FB.getUserID(), name:window.name, text:$scope.text, date: (new Date()).getTime()});
                     name: $scope.name,
                     text: $scope.text
              });
	    $scope.text = '';
